...
$(function(){
    ...
    var trove_api_url = "http://api.trove.nla.gov.au/result?zone=book&l-format=Thesis&l-australian=y";
   ...
   function remove_dates(url) {
    ...
    function makeChart(type) {
        ...
        if (type == "total") {
            yLabel = "Number of theses matching query";
        } else if (type == "ratio") {
          ...
          title: {
           ...
           tooltip: {
              formatter: function() {
                    ...
                    if (type == "total") {
                        displayValue = this.y + " theses (" + (dataSources.sources[this.series.index].getRatio(year, month) * 100).toPrecision(2) + "% )";
                    } else if (type == "ratio") {
                        displayValue = (this.y * 100).toPrecision(2) + "% (" + dataSources.sources[this.series.index].getTotal(year, month) + " theses)";
                    }
                    if (interval == "month") {
                        return '<b>'+ this.series.name +'</b><br/>'+ month_name + ': ' + displayValue + '<br><em>Click to view theses.</em>';
                    } else {
                        return '<b>'+ this.series.name + '</b><br/>' + year +': ' + displayValue + '<br><em>Click to view theses.</em>';
    ...
    function show_trove_articles(results, query_date, series) {
        ...
        $('#articles').append('<div class="more"><p><a class="btn" target="_blank" href="' + dataSources.sources[series.index].web_query + '+date%3A%5B' + query_date + '+TO+' + query_date + '%5D">&gt; View more in Trove</a></p></div>');
